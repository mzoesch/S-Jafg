...
struct LLight final
{
    ...
    FORCEINLINE constexpr explicit LLight(const uint32 InColor) /* Does this work with big endian? */
    ...
    FORCEINLINE void operator=(const uint32 InColor) { this->R = (InColor & 0x000000FF) >>  0; this->G = (InColor & 0x0000FF00) >>  8; this->B = (InColor & 0x00FF0000) >> 16; }
    FORCEINLINE void operator=(const LLightDomainTy InChannels[3]) { this->R = InChannels[0]; this->G = InChannels[1]; this->B = InChannels[2]; }

    FORCEINLINE uint32 ToRGB() const { return (this->R <<  0) | (this->G <<  8) | (this->B << 16); }
    FORCEINLINE uint32 ToBGR() const { return (this->B <<  0) | (this->G <<  8) | (this->R << 16); }
    ...
    FORCEINLINE bool operator!=(const LLight& InLight) const { return this->R != InLight.R || this->G != InLight.G || this->B != InLight.B; }
    FORCEINLINE bool operator==(const uint32 InColor) const { return this->R == ((InColor & 0x000000FF) >>  0) && this->G == ((InColor & 0x0000FF00) >>  8) && this->B == ((InColor & 0x00FF0000) >> 16); }
    FORCEINLINE bool operator!=(const uint32 InColor) const { return this->R != ((InColor & 0x000000FF) >>  0) || this->G != ((InColor & 0x0000FF00) >>  8) || this->B != ((InColor & 0x00FF0000) >> 16); }

    FORCEINLINE void operator+=(const LLight& InLight)
    {
        this->R += static_cast<LLightDomainTy>(Maths::Min(static_cast<uint32>(this->R) + static_cast<uint32>(InLight.R), 0xFFu));
        this->G += static_cast<LLightDomainTy>(Maths::Min(static_cast<uint32>(this->G) + static_cast<uint32>(InLight.G), 0xFFu));
        this->B += static_cast<LLightDomainTy>(Maths::Min(static_cast<uint32>(this->B) + static_cast<uint32>(InLight.B), 0xFFu));
    }
    FORCEINLINE void operator-=(const LLight& InLight)
    {
        this->R -= static_cast<LLightDomainTy>(Maths::Max(static_cast<int32>(this->R) - static_cast<int32>(InLight.R), 0x00));
        this->G -= static_cast<LLightDomainTy>(Maths::Max(static_cast<int32>(this->G) - static_cast<int32>(InLight.G), 0x00));
        this->B -= static_cast<LLightDomainTy>(Maths::Max(static_cast<int32>(this->B) - static_cast<int32>(InLight.B), 0x00));
    }
