...
struct TPlane final
{
    ...
    TVec<L,TReal,Q> normal;
};
template<typename TReal,maths::qual_t Q> using TPlane1 = TPlane<1,TReal,Q>;
using LPlane1F = TPlane1<maths::single_precision,maths::defaultp>;
using LPlane1D = TPlane1<maths::double_precision,maths::defaultp>;
template<typename TReal,maths::qual_t Q> using TPlane2 = TPlane<2,TReal,Q>;
using LPlane2F = TPlane2<maths::single_precision,maths::defaultp>;
using LPlane2D = TPlane2<maths::double_precision,maths::defaultp>;
template<typename TReal,maths::qual_t Q> using TPlane3 = TPlane<3,TReal,Q>;
using LPlane3F = TPlane3<maths::single_precision,maths::defaultp>;
using LPlane3D = TPlane3<maths::double_precision,maths::defaultp>;
template<typename TReal,maths::qual_t Q> using TPlane4 = TPlane<4,TReal,Q>;
using LPlane4F = TPlane4<maths::single_precision,maths::defaultp>;
...
using LWorldRect1   = TRect1<LWorldReal,world_qual>;
using LWorldRect2   = TRect2<LWorldReal,world_qual>;
using LWorldRect3   = TRect3<LWorldReal,world_qual>;
using LWorldPlane1  = TPlane1<LWorldReal,world_qual>;
...
#define MATHS_FLT_CONSTANT_V(Constant, Value)                                                              \
    ...
    inline constexpr f32 JAFG_JOIN_OUTER_TWO(Constant, _f){JAFG_JOIN_OUTER_TWO(Constant,_v)<f32>}; \
    inline constexpr f64 JAFG_JOIN_OUTER_TWO(Constant, _d){JAFG_JOIN_OUTER_TWO(Constant,_v)<f64>};
...
NODISCARD constexpr octant get_octant(TVec3<TReal,Q> a, TVec3<TReal,Q> b) noexcept
{
    auto d{b - a};
    std::underlying_type_t<octant> r{};
    if (d.x < 0)
    {
        r |= 1;
    }
    ...
    return static_cast<octant>(r);
}

template<typename TReal,qual_t Q>
NODISCARD constexpr octant get_angled_octant(TVec3<TReal,Q> const& a, TQua<TReal,Q> const& q, TVec3<TReal,Q> const& b) noexcept
{
    auto d{glm::conjugate(q) * (b - a)};
    std::underlying_type_t<octant> r{};
    if (d.x < TReal{0})
    {
        r |= 1;
...
//#
//# Given two lines A and B, find the closest delta on A to B.
//# Returns zero if parallel.
//#
template<std::floating_point TReal, qual_t Q>
NODISCARD constexpr TReal closest_delta_on_line(TRay3<TReal,Q> const& A, TRay3<TReal,Q> const& B, TReal epsilon = static_cast<TReal>(not_so_small_number_d)) noexcept
{
    TVec3<TReal,Q> u{A.direction};
    TVec3<TReal,Q> v{B.direction};
...
//#
//# Given two lines A and B, find the closest point on A to B.
//# Returns the origin if parallel.
//#
template<std::floating_point TReal, qual_t Q>
...
{
    return A.origin + closest_delta_on_line(A,B,epsilon) * A.direction;
}

//#
//# Given two rays A and B, find the closest point on A to B.
//# Unlike lines, rays are semi-infinite, as they are clamped to [origin, inf).
//# Returns zero if parallel.
//#
template<std::floating_point TReal, qual_t Q>
NODISCARD constexpr TReal closest_delta_on_ray(TRay3<TReal,Q> const& A, TRay3<TReal,Q> const& B, TReal epsilon = static_cast<TReal>(not_so_small_number_d)) noexcept
{
    TVec3<TReal,Q> u{A.direction};
    TVec3<TReal,Q> v{B.direction};
    TVec3<TReal,Q> w{A.origin - B.origin};
...
{
    return A.origin + closest_delta_on_ray(A,B,epsilon) * A.direction;
}

//#
//# Given a line A and a plane B, find the intersection delta to the origin of A to B.
//# Returns zero if parallel.
//#
template<std::floating_point TReal, qual_t Q>
NODISCARD constexpr TReal intersection_delta_on_plane(TRay3<TReal, Q> const& A, TPlane3<TReal, Q> const& B, TReal epsilon = static_cast<TReal>(not_so_small_number_d)) noexcept
{
    TReal denom{dot(B.normal, A.direction)};
    /* Parallel */
    if (abs(denom) < epsilon)
    {
        return TReal{0};
    }
    return dot(B.normal, B.origin - A.origin) / denom;
}

//#
//# Given a line A and a plane B, find the intersection point of A to B.
//# Returns the origin if parallel.
//#
template<std::floating_point TReal, qual_t Q>
NODISCARD constexpr TReal intersection_point_on_plane(TRay3<TReal, Q> const& A, TPlane3<TReal, Q> const& B, TReal epsilon = static_cast<TReal>(not_so_small_number_d)) noexcept
{
    return A.origin + intersection_delta_on_plane(A, B, epsilon) * A.direction;
}
...
{
    return translate(maths::identity<TMat4<T,Q>>, trans.t)
        *  mat4_cast(trans.r)
        *  scale(maths::identity<TMat4<T,Q>>, trans.s);
}
template<typename T,qual_t Q>
inline constexpr TMat4<T,Q> look_at(TVec3<T,Q> eye, TVec3<T,Q> target, TVec3<T,Q> up) noexcept
{
    TVec3<T,Q> forward{normalize(target - eye)};
    TVec3<T,Q> right{normalize(cross(forward, up))};
    TVec3<T,Q> newUp{cross(right, forward)};

    auto view{identity<TMat4<T,Q>>};
    ...
    view[3][0] = -dot(right, eye);
    view[3][1] = -dot(newUp, eye);
    view[3][2] =  dot(forward, eye);

    return view;
}
template<typename T,qual_t Q>
inline constexpr TMat4<T,Q> perspective(T vertical_fov, T aspect, T near, T far) noexcept
{
    return glm::perspectiveRH_ZO(vertical_fov, aspect, near, far);
}
...
{
    return
        angle_axis(yaw(rads), up_vector<TVec3<T,Q>>) *
        angle_axis(pitch(rads), right_vector<TVec3<T,Q>>) *
        angle_axis(roll(rads), forward_vector<TVec3<T,Q>>);
}
template<typename T,qual_t Q=highp> requires std::is_floating_point_v<T>
NODISCARD constexpr TQua<T,defaultp> rotator(T pitch, T yaw, T roll) noexcept
{
    return
        angle_axis(yaw, up_vector<TVec3<T,Q>>) *
        angle_axis(pitch, right_vector<TVec3<T,Q>>) *
        angle_axis(roll, forward_vector<TVec3<T,Q>>);
}
...
template<typename T, qualifier Q>
NODISCARD constexpr TVec3<T,Q> editor_euler_angles(TQua<T, Q> const& q, T e = not_so_small_number_v<T>) noexcept
{
    TMat<3,3,T,Q> m{glm::mat3_cast(q)};

    T s{clamp(-m[2][1], T{-1}, T{1})};
    T pitch{asin(s)};
    T cp{std::cos(pitch)};
...
template<typename T,qual_t Q>
NODISCARD constexpr TVec3<T,Q> editor_euler_angles_deg(TQua<T,Q> const& q) noexcept
{
    return degrees(editor_euler_angles(q));
}
...
NODISCARD inline decltype(auto) to_string(octant o) noexcept
{
    return std::vformat("{}{}{}", std::make_format_args(
          std::to_underlying(o) & 1 ? "-" : "+"
...
template<std::floating_point TReal,qual_t Q>
NODISCARD TRay<3,TReal,Q> screen_to_world_space(TVec2<TReal,Q> extent, TVec2<TReal,Q> location, TEye<TReal,Q> const& eye) noexcept
{
    ...
    return TRay<3,TReal,Q>{.origin = NearWorld, .direction = normalize(FarWorld - NearWorld)};
}
