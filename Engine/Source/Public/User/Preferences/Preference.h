...
public:
    ...
    FORCEINLINE LPreference(const LName InName) noexcept
        : Name(InName), DisplayName(algo::add_spaces_to_camel_case(InName.ToString())) { }
    FORCEINLINE LPreference(const LName InName, LBuildPreference&& InBuildDelegate) noexcept
        : Name(InName), DisplayName(algo::add_spaces_to_camel_case(InName.ToString())), OnBuildDelegate(std::move(InBuildDelegate)) { }
