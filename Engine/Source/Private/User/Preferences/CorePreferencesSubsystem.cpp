...
void Jafg::JCorePreferencesSubsystem::Initialize(LSubsystemCollection& Collection)
{
...

    {
        ...
        Screen->OnLoad([](LPreferenceCollection* InCollection) -> void
        {
            ...
            {
                // TUnique<LPreferenceValue_InputAction> T = std::make_unique<LPreferenceValue_InputAction>(Action.GetTag());
        ...
        Screen->OnLoad([](LPreferenceCollection* InCollection) -> void
        {
            ...
            LCommandLineInterface& Cli = GEngine->GetCommandLineInterface();
            {
                ...
                for (const LCliType& Type : Cli.GetTypes())
                {
                    ...
                    (
                        MAKE_NAME(Jafg::SprintF("CliType_{}", Type.GetIdentifier())),
                        Jafg::SprintF("Cli Type {}", Type.GetIdentifier()),
                        Handle.value()
                    );
...

            {
                ...
                for (const LCliCommand& Type : Cli.GetCommands())
                {
                    ...
                    (
                        MAKE_NAME(Jafg::SprintF("CliCmd_{}", Type.GetIdentifier())),
                        Jafg::SprintF("Cli Cmd {}", Type.GetIdentifier()),
                        Handle.value()
                    );
...

            {
                ...
                for (const LCliVariable& Type : Cli.GetVariables())
                {
                    ...
                    (
                        MAKE_NAME(Jafg::SprintF("CliVar_{}", Type.GetIdentifier())),
                        Jafg::SprintF("Cli Var {}", Type.GetIdentifier()),
                        Handle.value()
                    );
