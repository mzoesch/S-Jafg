...
void Jafg::WTextButton::Draw(LNodeRenderInfo const& Info) const
{
    ...
    if (this->Icon.get() && this->IconBrush.Scale > 0.0f)
    {
        ...
        f32 TotalHeight{(this->GetAnchoredSize_v2() - this->GetPadding().GetDesiredSizeInSpt(this->GetViewport())).y};
        LVec2F TopLeft{
        ...
        Info.VisualInstances.emplace_back(LVisualInstance{
            .Rect = {maths::floor(TopLeft), this->Icon->GetExtentAsVec2F() * static_cast<f32>(this->IconBrush.Scale)},
            .Tint = this->IconBrush.Tint.ToVector4(),
            .BackgroundTint = Colors::Black.ToVector4(),
            .Radii = maths::zero_vector<LVec4F>,
            .OutlineTint = maths::zero_vector<LVec4F>,
