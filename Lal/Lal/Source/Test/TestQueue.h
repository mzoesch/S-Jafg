...
TEST_CASE(SpScQueueOperations, "Lal.Containers")
{
    ...
    CHECK_EQUALS(   "Queue with zero size.", Queue.UnsafeSize(),  0 )
    CHECK_EQUALS(   "Queue with zero size.", Queue.Head, Queue.Tail )
    CHECK_NOT_NULL( "Queue with zero size.", Queue.Head             )

    Queue.Enqueue(5);
    CHECK_FALSE(      "Queue with one item.", Queue.IsEmpty()        )
    CHECK_EQUALS(     "Queue with one item.", Queue.UnsafeSize(),  1 )
    ...
    CHECK_EQUALS(   "Queue with five items.", Queue.UnsafeSize(),  5 )
    ...
    CHECK_EQUALS( "Queue with four items.", Queue.UnsafeSize(),  4 )
