...
// MakeVslfModuleProjects creates the VSLF projects that are used by module-specific pch files.
func MakeVslfModuleProjects() {
    fmt.Println("Making VSLF module projects...")
    for _, proj := range Shared.GApp.Projects {
        for _, mod := range proj.Modules {
            if mod.Pch.IsUse() {
                MakeVslfModuleForModule(&mod)
            }
...
func MakeVslfModuleForModule(mod *Shared.Module) {
    if mod == nil {
        panic("MakeVslfModuleForModule called with nil module.")
    }

    var targetVslfDir string = GetVslfModuleProjectRelativeDir(mod)
    var targetVslfFile string = fmt.Sprintf("%s/%s%s",
        targetVslfDir, mod.GetUsableName(), Shared.GeneratedTranslationsExtension)

    var vslfContent string = fmt.Sprintf(`
...
/**
 * Include file for generating module-specific precompiled header file.
 * Targeted module: %s
 */
#include "CoreAfx.h"
...
/*-----------------------------------------------------------------------------
    END Generated translation content.
-----------------------------------------------------------------------------*/
...
`,
    ...
    Shared.CheckRelativeDir(targetVslfDir)
    Shared.CheckRelativeFile(targetVslfFile)

    var file *os.File = Shared.OpenRelativeFile(targetVslfFile, false, os.O_RDWR|os.O_CREATE)
    if Shared.IsFileAndStringEqual(file, vslfContent, true) {
        fmt.Printf("VSLF module project for module [%s] is up to date. Skipping re-write.\n", mod.GetUsableName())
    } else {
        fmt.Printf("VSLF module project for module [%s] is outdate. Writing new content.\n", mod.GetUsableName())
        Shared.TruncateFile(file)
        Shared.SeekFileBeginning(file)
        Shared.WriteToFile(file, vslfContent)
    }
...
func WriteLuaBuildFileForSpecificProject(builder *strings.Builder, indent int, proj *Shared.Project) {
    ...
    //
    //    WriteWithIndent(builder, indent+4, fmt.Sprintf("project 'Vslf-%s'\n", mod.GetUsableName()))
    //
    //    WriteWithIndent(builder, indent+8, fmt.Sprintf("location '%s'\n", GetVslfModuleProjectRelativeDir(&mod)))
    //    WriteWithIndent(builder, indent+4, fmt.Sprintf("kind '%s'\n", "StaticLib"))
    //
    //    WriteWithIndent(builder, indent+8, fmt.Sprintf(
    //        "targetdir ('%s/%%{cfg.system}-%%{cfg.architecture}/%%{cfg.buildcfg}/%s/')\n",
    //        Shared.BinariesDir, GetVslfModuleProjectRelativeDir(&mod),
    //    ))
    //    WriteWithIndent(builder, indent+8, fmt.Sprintf(
    //        "objdir ('%s/%%{cfg.system}-%%{cfg.architecture}/%%{cfg.buildcfg}/%s/')\n",
    //        Shared.IntermediateDir, GetVslfModuleProjectRelativeDir(&mod),
    //    ))
    //
    //    WriteWithIndent(builder, indent+4, fmt.Sprintf("rtti '%s'\n", "Off"))
    //
    ...
    //    var lalMod *Shared.Module = Shared.GApp.GetModuleByName("Lal")
    //    includeDirs = append(includeDirs, Shared.GeneratedHeadersDir)
    //    includeDirs = append(includeDirs, fmt.Sprintf("%s/Source/Public", lalMod.GetRelativeModuleDir()))
    //    linkedLibs = append(linkedLibs, lalMod.GetUsableName())
    //
    //    for _, targ := range Shared.GApp.GetAllTargets() {
    //        for _, buildConfiguration := range GetAllBuildTargetConfigurations(targ) {
    ...
    //    continue
    //}
    //WriteWithIndent(builder, indent, fmt.Sprintf("group ''\n"))
